...
plan: 03
subsystem: backup
tags: [backup, retention, cleaning-orchestrator, cancellation, state, testing]

# Dependency graph
requires:
  - phase: 07-backup-restore-retention-safety
    provides: async backup/restore/retention result contracts and structured retention outcomes from Plans 07-01 and 07-02
provides:
...
affects: [cleaning-orchestrator, progress-ui, backup-retention, restore-ui]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - separate linked CancellationTokenSource ownership for non-xEdit file operations
    - state-published BackupOperationState for backup/retention progress without reusing xEdit stop state
    - TDD RED/GREEN commits for state and orchestrator behavior
...
key-decisions:
  - "Represent backup and retention cleanup as separate AppState.BackupOperation state so the UI can add a non-xEdit cancel affordance without reusing xEdit Stop semantics."
  - "Use a short-lived linked CancellationTokenSource only while backup or retention file work is active, guarded separately from the session CTS and process lock."
  - "Map canceled backup copies to a skipped plugin result with `Backup canceled` and skip xEdit launch for that plugin while allowing the session to continue."

patterns-established:
  - "Backup/retention progress is set and cleared through IStateService rather than by mutating ViewModel state directly."
  - "CancelBackupOperationAsync is idempotent and no-ops while xEdit is running, preserving existing Stop/ForceStop process semantics."
  - "Retention cleanup warnings/cancellations are attached to CleaningSessionResult before FinishCleaningWithResults emits completion."
