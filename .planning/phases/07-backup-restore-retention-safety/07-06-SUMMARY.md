...
plan: 06
subsystem: backup
tags: [backup, cleaning-orchestrator, session-results, testing, safety]

# Dependency graph
requires:
  - phase: 07-backup-restore-retention-safety
    provides: backup failure callback integration and state publication from Plan 07-03
provides:
...
affects: [cleaning-orchestrator, backup-failure-reporting, phase-07-verification]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - TDD RED/GREEN commits for backup failure choice regressions
    - branch-local session finalization for early abort control flow
    - explicit skipped PluginCleaningResult publication before continuing plugin loop
...
key-decisions:
  - "Publish BackupFailureChoice.SkipPlugin as a skipped PluginCleaningResult instead of only mutating SkippedPlugins."
  - "Finalize BackupFailureChoice.AbortSession inside the backup-failure branch because it returns before the normal end-of-method session finalization path."

patterns-established:
  - "Backup failure user choices produce observable state rows and final session results before control flow skips or exits."
  - "Abort-on-backup-failure preserves already processed plugin results in the final canceled CleaningSessionResult."
