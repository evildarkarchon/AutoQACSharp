...
plan: 05
subsystem: ui
tags: [backup, retention, progress-ui, avalonia, cancellation, testing]

# Dependency graph
requires:
  - phase: 07-backup-restore-retention-safety
    provides: backup/retention operation state and cancellation API from Plan 07-03 plus restore UX tests from Plan 07-04
provides:
...
affects: [progress-ui, cleaning-orchestrator, backup-retention, restore-ui, phase-08-readiness]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - CommunityToolkit.Mvvm derived state for active non-xEdit file operations
    - Avalonia progress band with explicit backup/cleanup cancel buttons
    - source-level sequential-cleaning guard test as a secondary invariant
...
key-decisions:
  - "Reuse ICleaningOrchestrator.CancelBackupOperationAsync for both Cancel Backup and Cancel Cleanup so non-xEdit cancellation remains separate from Stop/ForceStop."
  - "Show backup/retention operation state as a compact progress band above plugin progress while preserving the existing hang-warning banner."
  - "Use exact required behavioral test names plus a source-level sequential guard to make Phase 7 verification auditable."

patterns-established:
  - "ProgressViewModel derives active-operation label, cancel visibility, and byte/count progress text from AppState.BackupOperation."
  - "ProgressWindow keeps xEdit Stop visible for xEdit cleaning and uses separate 44px-minimum file-operation cancel buttons."
  - "Final phase validation combines named behavioral tests with full solution build/test verification."
