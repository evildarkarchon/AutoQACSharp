...
plan: 13
subsystem: backup
tags: [backup, restore, delete-session, filesystem-safety, mvvm, service-layer-move, tdd, gap-closure]

# Dependency graph
requires:
  - phase: 07-backup-restore-retention-safety
    provides: shared BackupPathContainment.IsContained helper from Plan 07-14 (eliminates containment-logic duplication risk)
  - phase: 07-backup-restore-retention-safety
    provides: trusted restore root parameter, RestoreCommands_DisabledWhenTrustedRestoreRootMissing fail-closed UI gate, and Restore Selected/All gating from Plan 07-11
  - phase: 07-backup-restore-retention-safety
    provides: IBackupSessionDeleter seam from Plan 07-02 (recursive delete is testable via injected substitute)
  - phase: 07-backup-restore-retention-safety
    provides: original RestoreWindow Delete Session command + RestoreViewModel.DeleteSessionAsync from Plan 07-04
provides:
...
affects: [backup-service, restore-ui, mvvm-boundaries, delete-session-policy, phase-07-verification-truth-20]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - service-layer ownership of recursive filesystem deletion with structured-result return semantics (no expected-failure exceptions)
    - shared containment helper consumption from both BackupService and ViewModel callers via the canonical IsContained policy
    - command predicate gating on private fields combined with explicit NotifyCanExecuteChanged() after mutation
    - canonical user-facing sentence pattern for safety boundaries (one sentence in StatusText AND dialog details)
    - TDD RED/GREEN gates committed atomically with deliberate compile-time RED on missing types

key-files:
  created:
    - .planning/phases/07-backup-restore-retention-safety/07-13-SUMMARY.md
  modified:
...
key-decisions:
  - "RestoreWindow Delete Session containment policy lives in IBackupService.DeleteSessionAsync, not in the ViewModel. Filesystem work runs through the existing IBackupSessionDeleter seam. This resolves the cross-AI reviewer consensus (the agent + Codex MEDIUM-HIGH; Gemini accepting) that ViewModels must not call System.IO.Directory.Delete when an injectable service seam already exists."
  - "DeleteSessionAsync delegates string-level containment validation to BackupPathContainment.IsContained from Plan 07-14, eliminating the helper-duplication risk (HIGH from the agent + Codex; Gemini consistent-normalization)."
  - "DeleteSessionCommand.CanExecute is gated on _backupRoot non-null/non-whitespace AND HasTrustedRestoreRoot AND !IsRestoreActive, with a defense-in-depth top-of-method guard for direct ExecuteAsync(null) callers."
  - "One canonical sentence ('The selected backup session is outside the configured backup folder.') is shared between StatusText and dialog details for the out-of-root branch; generic 'Technical details were written to the log.' is used for IO failures so exception messages never reach the user (D-04 concise reason pattern)."

patterns-established:
  - "Recursive filesystem deletion belongs in IBackupService, not RestoreViewModel; the ViewModel surfaces structured outcomes only."
  - "When a private (non-[ObservableProperty]) field gates a [RelayCommand] predicate, the mutation site must call DeleteSessionCommand.NotifyCanExecuteChanged() explicitly because the source generator only re-evaluates predicates on observable property changes."
  - "Canonical safety-boundary sentences are reused verbatim across StatusText and dialog details; do not reformat or prefix them."
