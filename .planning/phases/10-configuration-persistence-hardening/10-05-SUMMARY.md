...
plan: 05
subsystem: ui
tags: [csharp, avalonia, settings-viewmodel, configuration-persistence, xunit]

requires:
  - phase: 10-configuration-persistence-hardening
    provides: typed ConfigPersistenceFailure, PersistenceResults, and ConfigWatcherService coordinator wiring from Plans 02 and 03
  - phase: 10-configuration-persistence-hardening
    provides: pre-cleaning flush failure blocker from Plan 04
provides:
...
affects: [settings-ui, configuration-persistence, phase-11-diagnostics]

tech-stack:
  added: []
  patterns:
    - CallbackObserver<T> subscriptions in ViewModels marshaled through IUiDispatcher
    - Text-only recoverable error banner with no modal dialog and no retry UI
    - Flush-before-close barrier for explicit Settings saves
