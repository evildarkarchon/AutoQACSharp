...
tags: [configuration-persistence, watcher-reload, read-failure, tdd, race-regression]

requires:
  - phase: 10-09
    provides: Forced facade flushes always drain the coordinator queue barrier before cleaning continues
provides:
...
affects: [configuration-persistence-hardening, watcher-reload, REF-03, TEST-03]

tech-stack:
  added: []
  patterns:
    - Watcher hash acquisition is treated as live file reading and participates in the typed recoverable failure path
    - Fake file-store hash failure injection makes writer-lock races deterministic without FileSystemWatcher timing
