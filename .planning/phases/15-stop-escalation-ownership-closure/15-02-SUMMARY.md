...
tags: [csharp, cleaning, process-termination, progress-window, tdd, xunit]

requires:
  - phase: 15-stop-escalation-ownership-closure
    provides: retained pending force-escalation process ownership in CleaningTerminationCoordinator
provides:
...
tech-stack:
  added: []
  patterns: [separate session finalization cleanup from new-session reset, service-level detached escalation regression]
