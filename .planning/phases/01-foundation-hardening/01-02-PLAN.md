...
**NOTE:** This is lightweight foundation infrastructure for the locked user decision "Block all controls with a 'Stopping...' spinner while termination is in progress" (from 01-CONTEXT.md). No UI implementation is needed in this plan -- only the observable plumbing. The actual "Stopping..." spinner UI will be implemented in a future plan that consumes this observable.

Add a new property to track whether termination is in progress:
