...
requires:
  - phase: 11-user-facing-diagnostics-boundaries
    provides: Shared DiagnosticTextFormatter safe text boundary from Plan 11-01
provides:
...
affects: [cleaning-command-ui, diagnostics-boundaries, sec-01]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - ViewModel command catches log technical exceptions while showing DiagnosticTextFormatter safe copy
    - Missing-path validation rows use sanitized setting/resource basenames with direct fix guidance only
