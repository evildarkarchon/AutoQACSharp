...
must_haves:
  ...
  key_links:
    - from: "AutoQAC.Tests/Services/BackupServiceTests.cs"
      ...
      via: "Tests use temp directories for file system isolation"
...
<objective>
Add comprehensive test coverage for phase 1-6 features (TEST-06), update Mutagen dependencies to latest stable (DEP-01, DEP-02), and remove Code_To_Port/ reference directory with CLAUDE.md cleanup (POST-01).
...
Output: 3 new test files, updated Mutagen packages, Code_To_Port/ removed from repo, CLAUDE.md cleaned of porting references.
</objective>
...
BackupService constructor takes `ILoggingService` (use Mock.Of<ILoggingService>()). All file operations use real temp directories. Implement IDisposable for cleanup.
...
LogRetentionService constructor takes `IConfigurationService` and `ILoggingService`. It uses a hardcoded "logs" directory path (relative to working directory). This makes testing challenging.

IMPORTANT: Read the implementation carefully. The service uses `Directory.Exists("logs")` and `Directory.GetFiles("logs", "autoqac-*.log")`. Since this uses a relative path, tests need to either:
- Set the working directory to a temp dir before each test, OR
- Accept that the service uses the current directory's "logs" subfolder

The simplest approach: Create a "logs" directory in the test's current working directory, populate it with test log files, run the test, then clean up. But this risks interference between parallel test runs.

Better approach: Since LogRetentionService uses a hardcoded path, the tests should create/manage a "logs" directory relative to CWD. Use IDisposable to track and clean up created files. Use unique filenames per test to avoid collisions.

Alternatively: If the hardcoded path makes testing too brittle, note this limitation and test through the public API as best as possible. Focus on the testable paths.
...
XEditLogFileService constructor takes `ILoggingService`. All methods are pure logic + file I/O.
...
<task type="auto">
  <name>Task 2: Dependency updates, Code_To_Port removal, and CLAUDE.md cleanup</name>
  <files>
    AutoQAC/AutoQAC.csproj
    CLAUDE.md
  </files>
  <action>
**Step 1: Update Mutagen packages (DEP-01)**
...
**Step 2: Check all other packages for updates (DEP-02)**
...
**Step 3: Remove Code_To_Port/ directory (POST-01)**
...
5. CLAUDE.md contains no references to "Code_To_Port", "porting", "reference implementation", or the translation table
  ...
  <done>
Mutagen packages updated to 0.53.1 with any breaking changes fixed. All NuGet packages are at latest compatible versions. Code_To_Port/ directory removed from repository. CLAUDE.md cleaned of all porting references and translation tables. Project builds and all tests pass.
  </done>
</task>
