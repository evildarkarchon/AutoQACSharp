...
tags: [queryplugins, mutagen, itm, cancellation, tdd]

# Dependency graph
requires:
  - phase: 08-cleaning-orchestrator-decomposition
    provides: stable cleaning orchestration boundaries before refresh performance work
provides:
...
affects: [plugin-refresh, approximation-refresh, queryplugins, perf-02, perf-01]

# Tech tracking
tech-stack:
  added: []
  patterns: [cooperative-cancellation, streaming-mutagen-contexts, red-green-tdd]
...
key-decisions:
  - "Kept QueryPlugins count semantics exact by streaming only the analyzed plugin context and its immediate lower-priority context."
  - "Propagated OperationCanceledException instead of publishing unavailable or partial rows when exact analysis is canceled."

patterns-established:
  - "QueryPlugins analysis contracts accept an optional CancellationToken as the final parameter."
  - "ITM detection polls cancellation in both major-record and link-cache context loops."
