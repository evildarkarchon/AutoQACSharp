...
requires:
  - phase: 08-cleaning-orchestrator-decomposition
    provides: Phase 8 collaborator extraction through Plan 08-06
provides:
...
affects: [cleaning-orchestrator, stop-semantics, preflight, process-cleanup]

# Tech tracking
tech-stack:
  added: []
  patterns: [TDD RED/GREEN gap closure, early session CTS publication]
...
key-decisions:
  - "Create and publish the cleaning session CTS before orphan cleanup/preflight so StopCleaningAsync can cancel the startup window."
  - "Keep StopCleaningAsync and ForceStopCleaningAsync public behavior unchanged; only StartCleaningAsync ordering changed."

patterns-established:
  - "Startup-window cancellation tests block the awaited operation, call StopCleaningAsync, then assert no plugin cleaning occurred."
  - "CR-01 WHY-comments document the non-obvious CTS ordering constraint."
