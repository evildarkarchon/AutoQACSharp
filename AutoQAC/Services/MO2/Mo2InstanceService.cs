    ...
    : IMo2InstanceService
{
    ...
    {
        if (!Directory.Exists(instance.ProfilesDirectory)) return [];
    ...
    public string? ChooseProfile(Mo2InstanceInfo instance, IReadOnlyList<string> profiles, string? persistedProfile)
    {
        ...
        if (defaultProfile is not null) return defaultProfile;
    ...
    public string? GetLoadOrderPath(Mo2InstanceInfo instance, string profile)
    {
        ...
        if (File.Exists(loadOrder)) return loadOrder;
        ...
        string? gameDataFolder)
    {
        ...
        foreach (var modName in enabledMods) AddPluginFiles(map, Path.Combine(instance.ModsDirectory, modName));
        ...
        CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(overrideBaseDir) && Directory.Exists(overrideBaseDir))
        {
            ...
            return BuildInstance(overrideBaseDir, File.Exists(iniPath) ? iniPath : null, false);
        }
        ...
        return BuildInstance(Path.GetDirectoryName(selected.Path) ?? string.Empty, selected.Path, true,
            selected.Values);
    }
    ...
    private IEnumerable<string> EnumerateCandidateIniFiles(string? mo2BinaryPath)
    {
        ...
        if (portable is not null && File.Exists(portable) && seen.Add(portable)) yield return portable;
        ...
        if (globalRoot is null || !Directory.Exists(globalRoot)) yield break;
    ...
    {
        if (string.IsNullOrWhiteSpace(mo2BinaryPath)) return null;
    ...
    private static bool MatchesGame(string? gameName, GameType game)
    {
        return game switch
        {
            ...
            GameType.FalloutNewVegas => gameName is not null &&
                                        (string.Equals(gameName, "New Vegas", StringComparison.OrdinalIgnoreCase) ||
                                         string.Equals(gameName, "Fallout New Vegas",
