...
public sealed class XEditCommandBuilder(IStateService stateService) : IXEditCommandBuilder
{
    ...
    {
        if (gameType == GameType.Unknown) return null; // Safety: refuse to build command without known game type
        ...
        {
            if (string.IsNullOrWhiteSpace(config.Mo2ExecutablePath)) return null;
        ...
        string xEditPath)
    {
        ...
        var xEditName = Path.GetFileNameWithoutExtension(xEditPath);
        if (xEditName.StartsWith("xEdit", StringComparison.OrdinalIgnoreCase)) args.Add(GetGameFlag(gameType));
    ...
    ///     MO2 owns a second parser boundary, so this is intentionally different from direct mode's raw
    ///     <see cref="ProcessStartInfo.ArgumentList" /> tokens.
    /// </summary>
    ...
    private static string GetGameFlag(GameType gameType)
    {
