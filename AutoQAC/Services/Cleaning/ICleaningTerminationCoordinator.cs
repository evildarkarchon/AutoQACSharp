...
///     state is reset before sessions via <see cref="ResetForNewSession" /> and finalized after sessions via
///     <see cref="CompleteSessionFinalization" />.
/// </summary>
public interface ICleaningTerminationCoordinator
{
    ...
    ///     This clears active-cleaning semantics only; an unresolved GracePeriodExpired target may remain retained for a later
    ///     confirmed force escalation.
    /// </summary>
    void DetachProcess();
    ...
    /// <summary>
    ///     Performs the confirmed force-stop path. Uses the active process first, then any retained pending target from
    ///     GracePeriodExpired,
    ...
    ///     <see cref="TerminationResult.GracePeriodExpired" /> pending force-escalation target until the user resolves it or
    ///     the next-session reset clears it.
    /// </summary>
