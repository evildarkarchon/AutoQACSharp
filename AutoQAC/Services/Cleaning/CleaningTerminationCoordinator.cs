...
public sealed partial class CleaningTerminationCoordinator : ICleaningTerminationCoordinator, IDisposable
{
    ...
    /// <inheritdoc />
    public TerminationResult? LastTerminationResult { get; private set; }
    ...
    /// <inheritdoc />
    public bool ProcessMayStillBeRunning => MayProcessStillBeRunning(LastTerminationResult);
    ...
    ///     The pending target is separate ownership for a user-confirmed escalation after
    ///     <see cref="TerminationResult.GracePeriodExpired" />.
    /// </summary>
        ...
        if (proc != null)
            try
            {
                if (proc.Id == Environment.ProcessId)
                {
                    ...
                    return new StopCleaningResult(null, false);
                }

                if (!proc.HasExited)
                {
                    var result = await _processService
                        .TerminateProcessAsync(proc, false, CancellationToken.None)
                        .ConfigureAwait(false);
                    LastTerminationResult = result;
            ...
            catch (InvalidOperationException)
            {
                ...
                LastTerminationResult = TerminationResult.AlreadyExited;
                return ToStopCleaningResult(TerminationResult.AlreadyExited);
            }

        _stateService.SetTerminating(false);
        return new StopCleaningResult(LastTerminationResult, MayProcessStillBeRunning(LastTerminationResult));
    }

    /// <summary>
    ///     Performs a confirmed force stop against the active process or a retained pending force target.
    ///     Confirmed escalation never returns cached <see cref="TerminationResult.GracePeriodExpired" /> when a pending target
    ...
    public async Task<StopCleaningResult> ForceStopAsync()
    {
        ...
        try
        {
            ...
            {
                proc = _currentProcess;
                if (proc is null && MayProcessStillBeRunning(LastTerminationResult))
                {
            ...
            if (proc is null && pendingTarget is not null)
            {
                ...
                if (proc is null)
                {
                    ...
                    LastTerminationResult = TerminationResult.ForceKillFailed;
            ...
            if (proc != null)
                try
                {
                    ...
                    {
                        var result = await _processService
                            .TerminateProcessAsync(proc, true, CancellationToken.None)
                            .ConfigureAwait(false);
                        LastTerminationResult = result;
                ...
                {
                    if (disposeReopenedProcess) proc.Dispose();
                }

            if (MayProcessStillBeRunning(LastTerminationResult))
            {
        ...
        {
            if (!MayProcessStillBeRunning(LastTerminationResult)) _stateService.SetTerminating(false);
        }
    }
    ...
    {
        ReleasePendingForceEscalationTarget();
        LastTerminationResult = TerminationResult.LeftRunningByUser;
    ...
    /// <summary>
    ///     Clears active session state after normal finalization without erasing an unresolved grace-expired pending force
    ///     target.
    /// </summary>
    ...
    {
        return new StopCleaningResult(result, MayProcessStillBeRunning(result));
    }

    private static bool MayProcessStillBeRunning(TerminationResult? result)
    {
        return result is TerminationResult.GracePeriodExpired or TerminationResult.LeftRunningByUser
            or TerminationResult.ForceKillFailed;
    }

    /// <summary>
    ///     Captures durable process identity for a later confirmed force escalation without retaining the borrowed process
    ///     handle.
    /// </summary>
    ...
    private void ReleasePendingForceEscalationTarget(PendingForceTarget? target = null)
    {
        lock (_processLock)
        {
            if (target is null || Equals(_pendingForceEscalationTarget, target)) _pendingForceEscalationTarget = null;
        }
    }
